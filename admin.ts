...
function animateCounter(element: HTMLElement) {
    ...
    const timer = setInterval(() => {
        start += increment;
        if (start >= target) {
            element.textContent = isFloat ? target.toFixed(1) : String(target);
            clearInterval(timer);
        } else {
            element.textContent = isFloat ? start.toFixed(1) : String(Math.floor(start));
        }
    }, stepTime);
}
...
async function loadPendingVolunteers() {
    ...
    document.querySelectorAll('.btn-approve').forEach(button => {
        button.addEventListener('click', async (e) => {
            ...
            await approveVolunteer(volunteerId, e.target as HTMLButtonElement);
        });
    });
    ...
    document.querySelectorAll('.btn-reject').forEach(button => {
        button.addEventListener('click', async (e) => {
            const row = (e.target as HTMLElement).closest('tr');
            const volunteerId = row.dataset.id;
            const volunteerName = (row.cells[0] as HTMLElement).textContent;
            ...
            if (confirm(`¿Estás seguro de que quieres rechazar y eliminar la solicitud de ${volunteerName}? Esta acción no se puede deshacer.`)) {
                ...
                (row.querySelector('.btn-approve') as HTMLButtonElement).disabled = true;

                await rejectVolunteer(volunteerId, e.target as HTMLButtonElement);
            }
        });
    });
}

async function approveVolunteer(volunteerId: string, buttonElement: HTMLButtonElement) {
...
async function rejectVolunteer(volunteerId: string, buttonElement: HTMLButtonElement) {
    ...
    if (error) {
        ...
        (buttonElement.closest('tr').querySelector('.btn-approve') as HTMLButtonElement).disabled = false;
    } else {
